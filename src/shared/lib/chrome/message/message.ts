...
> {
  constructor(
    ...
    private readonly listenerMap: Map<
      keyof ListenerMap,
      Types.MessageListener<Types.MessageSignature<unknown, unknown>>
    >
  ) {}
  ...
  readonly setListener = (listener: Types.MessageListener<ListenerMap[Type]>): void => {
    this.listenerMap.set(
      this.type,
      listener as Types.MessageListener<Types.MessageSignature<unknown, unknown>>
    )
  }
...
export class Messenger<ListenerMap extends Types.BaseListenerMap> {
  ...
  constructor() {
    Chrome.runtime.onMessage.addListener(
      (
        message: { type: keyof ListenerMap; payload: unknown },
